...
describe("bikeNetworksService", () => {
    ...
    describe("getBikeNetworkById", () => {
        it("returns bike network detail on success", async () => {
            ...
            expect(fetch).toHaveBeenCalledWith(API_ROUTES.GET_BIKE_NETWORK_BY_ID("xyz"));
        });
        ...
        it("throws an error if response is not ok", async () => {
            ...
            await expect(bikeNetworksService.getBikeNetworkById("xyz")).rejects.toThrow(
                "Failed to fetch bike network with id xyz: Not Found"
            );
        });
    });
});
